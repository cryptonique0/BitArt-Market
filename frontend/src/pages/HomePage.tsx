  ...
  <section className="py-4 px-4 sm:px-6 lg:px-8">
    <div className="max-w-7xl mx-auto">
      <div className="mb-2 text-sm text-gray-700 dark:text-gray-200 font-semibold flex items-center gap-2">
        ...
        {aboutToLose && (
          <span className="px-2 py-1 rounded-full bg-amber-500 text-white animate-pulse">
      ...
      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 relative">
        ...
        />
        <div className="absolute left-2 top-0 h-4 flex items-center text-xs font-bold text-green-900 dark:text-green-100">
          {streak} / {streakGoal}
        </div>
        <div className="absolute right-2 top-0 h-4 flex items-center text-xs font-bold text-green-900 dark:text-green-100">
          {streakGoal}-day goal
        </div>
      </div>
    </div>
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 relative">
        <div
          ...
          style={{
            width: `${Math.min(100, (referralProgress.current / referralProgress.nextTier) * 100)}%`,
          }}
        />
        <div className="absolute left-2 top-0 h-4 flex items-center text-xs font-bold text-white">
          {referralProgress.current} / {referralProgress.nextTier}
        </div>
        <div className="absolute right-2 top-0 h-4 flex items-center text-xs font-bold text-purple-900 dark:text-purple-200">
          +{referralProgress.reward} ETH
        </div>
      </div>
    </div>
  </section>;
    ...
    <div className="max-w-7xl mx-auto flex gap-6 overflow-x-auto text-sm text-blue-900 dark:text-blue-100 items-center">
      {socialProof.map((e, i) => (
        <span key={i} className="whitespace-nowrap flex items-center gap-2">
          ...
          />
          <span>{e.user.slice(0, 6)}...</span> {e.action} <b>{e.nft}</b>{' '}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {quests.map(q => (
            <div
              key={q.id}
              className="p-4 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800"
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs px-2 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200">
                  {Math.max(
                    0,
                    Math.floor((new Date(q.expiresAt).getTime() - Date.now()) / 3600000)
                  )}
                  h left
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                <span className="px-2 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-200">
                  {q.badge}
                </span>
                ...
                {q.progress === q.target && (
                  <span className="ml-2 px-2 py-1 rounded-full bg-emerald-500 text-white animate-bounce">
                    Completed!
                  </span>
                )}
                {q.target - q.progress === 1 && q.progress < q.target && (
                  <span className="ml-2 px-2 py-1 rounded-full bg-amber-400 text-white animate-pulse">
                    Almost!
                  </span>
                )}
              </div>
          ...
          {quests.length === 0 && (
            <p className="text-gray-600 dark:text-gray-400">No active quests.</p>
          )}
        </div>
      </div>
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
        ...
        {portfolio ? (
          <>
            <div className="flex gap-4 mb-2">
              <span className="px-2 py-1 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-200 text-xs">
                Diversity {portfolio.diversityScore}
              </span>
              <span className="px-2 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-200 text-xs">
                Verified {portfolio.verifiedCollections}
              </span>
              <span className="px-2 py-1 rounded-full bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-200 text-xs">
                Unrealized PnL {portfolio.unrealizedPnL} ETH
              </span>
            </div>
            <ul className="list-disc pl-5 text-sm text-gray-700 dark:text-gray-200">
              {portfolio.tips.map((tip, i) => (
                <li key={i}>{tip}</li>
              ))}
            </ul>
          </>
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">
                Discover, create, and trade unique digital artworks on Base blockchain. Creators get
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
                <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                  ...
                  {user.chain === 'base' && user.address && (
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Balance: {baseBalance ? `${baseBalance} ETH` : 'Loading...'}
                    </p>
                  )}
                </div>
                ...
                <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-200">
                      Ready
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Create, mint, and trade NFTs on Base.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {mockTrendingNFTs.map(nft => (
              <div
                key={nft.id}
                className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow hover:shadow-lg transition-shadow cursor-pointer group"
              >
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Top collectors by XP, volume, streak
                </p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => setLbFilter('xp')}
                  className={`px-2 py-1 rounded text-xs font-semibold ${lbFilter === 'xp' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200'}`}
                >
                  XP
                </button>
                <button
                  onClick={() => setLbFilter('volume')}
                  className={`px-2 py-1 rounded text-xs font-semibold ${lbFilter === 'volume' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200'}`}
                >
                  Volume
                </button>
                <button
                  onClick={() => setLbFilter('streak')}
                  className={`px-2 py-1 rounded text-xs font-semibold ${lbFilter === 'streak' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200'}`}
                >
                  Streak
                </button>
                <button
                  disabled
                  className="px-2 py-1 rounded text-xs font-semibold bg-gray-100 dark:bg-gray-800 text-gray-400 cursor-not-allowed"
                >
                  Referrals
                </button>
              </div>
            ...
            {referrals ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="bg-white/10 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold">
                      {referrals.you.referredVolume.toFixed(1)} ETH
                    </p>
                  </div>
                  <div className="bg-white/10 rounded-lg p-3">
                ...
                <div className="text-sm text-white/80">
                  Top referrer: {referrals.topReferrers[0]?.username} ·{' '}
                  {referrals.topReferrers[0]?.volume.toFixed(1)} ETH
                </div>
                <button
              ...
              {topCreators.map((creator: { address: string; followerCount: number }) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    Engagement score powered by social signals
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="xl:col-span-2 space-y-8">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <section className="py-8 px-4 sm:px-6 lg:px-8">
                      <div className="max-w-7xl mx-auto flex flex-wrap gap-4">
                        <button
                          ...
                          onClick={() =>
                            alert(
                              'Bulk buy: All floor NFTs in your filters would be added to cart and purchased in one transaction (mock).'
                            )
                          }
                        >
                          Bulk Buy Floor
                        ...
                        <button
                          ...
                          onClick={() =>
                            alert(
                              'Bulk list: All your selected NFTs would be listed at floor price in one transaction (mock).'
                            )
                          }
                        >
                          Bulk List NFTs
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs">
                      {drop.tags.map(tag => (
                        <span
                          key={tag}
                          className="px-2 py-1 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                {drops.length === 0 && (
                  <p className="text-gray-600 dark:text-gray-400">No scheduled drops yet.</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {recommendations.map(rec => (
                  <div
                    key={rec.id}
                    className="flex gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800"
                  >
                ...
                {recommendations.length === 0 && (
                  <p className="text-gray-600 dark:text-gray-400">
                    Recommendations will appear once you collect or search.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                {!trustSignals && (
                  <p className="text-gray-600 dark:text-gray-400">Loading trust signals...</p>
                )}
              </div>
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-700 text-white rounded-2xl p-6 shadow-lg">
              ...
              <p className="text-sm text-white/80 mb-4">
                Earn XP for buys, sells, mints, listings, referrals. Streaks and safety badges boost
                your multiplier.
              </p>
              <button className="w-full py-3 bg-white text-slate-900 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
                Start a streak
              </button>
            </div>
          </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">
            Upload your artwork, set royalties, and start selling on BitArt Market. It takes just a
            few minutes.
          </p>
